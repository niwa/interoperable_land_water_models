...
namespace ApsimHandlerIOM
{
    ...
    public class MainHandler
    {
        ...
        /// <summary>List of names for the dimensions used to define the converted output database, using lat-long (a netcdf file).</summary>
        private string[] dimensionsLLNames = { "time", "latitude", "longitude" };
        /// <summary>List of units for the dimensions used to define the converted output database, using lat-long (a netcdf file).</summary>
        private string[] dimensionsLLUnits = { "number of days since 1959-12-31", "decimal degrees", "decimal degrees" };

        /// <summary>List of names for the dimensions used to define the converted output database, using location ID (a netcdf file).</summary>
        private string[] dimensionsIDNames = { "time", "location_id" };
        /// <summary>List of units for the dimensions used to define the converted output database, using location ID (a netcdf file).</summary>
        private string[] dimensionsIDUnits = { "number of days since 1959-12-31", "an identifier" };

        /// <summary>List of variables names to write down in the converted output database (a netcdf file).</summary>
        private string[] outputVariablesNames = { "yield", "leached_n" };
        /// <summary>List of variables long names, or descriptions, to write down in the converted output database (a netcdf file).</summary>
        private string[] outputVariablesLongNames = { "dry matter yield of crop or pasture at each harvest", "daily nitrate-nitrogen leached below the depth of 1.5m" };
        /// <summary>List of CDSM names for the variables to write down in the converted output database (a netcdf file).</summary>
        private string[] outputVariablesCDSMNames = { "crop_yield", "groundwater_N_loading_rate" };
        ...
        /// <summary>List of variable required from Apsim, corresponding to the dimensions (Lat-long) and outputVariables.</summary>
        private string[] apsimRequiredLLVariables = { "Date", "Latitude", "Longitude", "CropYield", "TotalLeachedN" };
        /// <summary>List of variable required from Apsim, corresponding to the dimensions (loc ID) and outputVariables.</summary>
        private string[] apsimRequiredIDVariables = { "Date", "LocationID", "CropYield", "TotalLeachedN" };
        ...
        ///  provided that the coordinates (based on file name) are unique.
        /// </remarks>
        ...
        ///  provided that the soils (based on names) are unique
        /// </remarks>
        /// <param name="locationName">Name of the location for which the data is to be prepared.</param>
        ...
        /// <param name="locationID">ID tag for the simulation's location (IOM).</param>
        ...
        public bool CreateApsimSimulation(string newSimulationName, string systemSetupName ="", string clockSettingName = "",
                                          string locationName = "", string locationID = "", bool verboseEnabled = false)
        {
            bool simOK = true;
            ...
            if (simOK)
            {
                ...
                foreach (dynamic apsimSimulation in mySimulations)
                {
                    ...
                    else if (newApsimLocationSettings != null)
                    {
                        locationIDList.Add(newApsimLocationSettings.Name);
                    }
                    else
                    {
                        locationIDList.Add("none");
                    }
                    setApsimModelParameters(apsimSimulation, "Manager", "AuxSummary", new List<string> { "TagOutput" }, locationIDList);
        ...
        public void SaveOutputsTable(string apsimDBFileName, string apsimDBTableName, string netCDFFileName, string outputsGroup = "LL")
        {
            System.Data.DataTable myData = null;
            if (outputsGroup == "LL")
            {
                myData = gatherSimulatedData(apsimDBFileName, apsimDBTableName, apsimRequiredLLVariables);
            }
            else if (outputsGroup == "ID")
            {
                myData = gatherSimulatedData(apsimDBFileName, apsimDBTableName, apsimRequiredIDVariables);
            }
            else
            {
                Console.WriteLine(" - Error: output data was not saved because the list of outputs was not recognised");
            }

            if (myData != null)
            {
                DataSet myNetCDFConnection = createNetCDFFileConnection(netCDFFileName, outputsGroup);
        ...
        /// <param name="outputsGroup">Flag identifying which group of Apsim outputs are to be saved in the netCDF file, LL or ID (optional).</param>
        ...
        public bool RunSimulationsAndSaveOutputs(string[] farmSystemsNames, string[] clockSettingNames, string[] locationNames,
                                                 ...
                                                 string outputsGroup = "LL", bool verboseEnabled = false)
        {
                ...
                for (int iLoc = 0; iLoc < locationNames.Length; iLoc++)
                {
                    for (int iClk = 0; iClk < clockSettingNames.Length; iClk++)
                    {
                        for (int iSys = 0; iSys < farmSystemsNames.Length; iSys++)
                        {
                            ...
                            processedOK = CreateApsimSimulation(thisSimulation + ".apsimx", farmSystemsNames[iSys], clockSettingNames[iClk], locationNames[iLoc], locationNames[iLoc], verboseEnabled);
                            simsToRun += 1;
                            ...
                            if (processedOK)
                            {
                                ...
                                if (processedOK)
                                {
                                    ...
                                    System.Data.DataTable thisData = null;
                                    if (outputsGroup == "LL")
                                    {
                                        thisData = gatherSimulatedData(thisSimulation + ".db", apsimDBTableName, apsimRequiredLLVariables);
                                        appendDataLLToNetCDF(myNetCDFConnection, thisData);
                                        simsRunOK += 1;
                                    }
                                    else if (outputsGroup == "ID")
                                    {
                                        thisData = gatherSimulatedData(thisSimulation + ".db", apsimDBTableName, apsimRequiredIDVariables);
                                        appendDataIDToNetCDF(myNetCDFConnection, thisData);
                                        simsRunOK += 1;
                                    }
                                    else
                                    {
                                                 ...
                                                 string outputsGroup = "LL", bool verboseEnabled = false)
        {
            ...
            return RunSimulationsAndSaveOutputs(farmSystemsArray, clockSettingNames, locationNames, simulationRootName, apsimDBTableName, netCDFFileName, outputsGroup, verboseEnabled);
        }
                                                 ...
                                                 string outputsGroup = "LL", bool verboseEnabled = false)
        {
            ...
            return RunSimulationsAndSaveOutputs(farmSystemsNames, clockSettingsArray, locationNames, simulationRootName, apsimDBTableName, netCDFFileName, outputsGroup, verboseEnabled);
        }
                                                 ...
                                                 string outputsGroup = "LL", bool verboseEnabled = false)
        {
            ...
            return RunSimulationsAndSaveOutputs(farmSystemsNames, clockSettingNames, locationsArray, simulationRootName, apsimDBTableName, netCDFFileName, outputsGroup, verboseEnabled);
        }
                                                 ...
                                                 string outputsGroup = "LL", bool verboseEnabled = false)
        {
            ...
            farmSystemsArray[0] = farmSystemsName;
            clockSettingsArray[0] = clockSettingName;
            return RunSimulationsAndSaveOutputs(farmSystemsArray, clockSettingsArray, locationNames, simulationRootName, apsimDBTableName, netCDFFileName, outputsGroup, verboseEnabled);
        }
                                                 ...
                                                 string outputsGroup = "LL", bool verboseEnabled = false)
        {
            ...
            farmSystemsArray[0] = farmSystemsName;
            locationsArray[0] = locationName;
            return RunSimulationsAndSaveOutputs(farmSystemsArray, clockSettingNames, locationsArray, simulationRootName, apsimDBTableName, netCDFFileName, outputsGroup, verboseEnabled);
        }
                                                 ...
                                                 string outputsGroup = "LL", bool verboseEnabled = false)
        {
            ...
            clockSettingsArray[0] = clockSettingName;
            locationsArray[0] = locationName;
            return RunSimulationsAndSaveOutputs(farmSystemsNames, clockSettingsArray, locationsArray, simulationRootName, apsimDBTableName, netCDFFileName, outputsGroup, verboseEnabled);
        }
                                                 ...
                                                 string outputsGroup = "LL", bool verboseEnabled = false)
        {
            ...
            clockSettingsArray[0] = clockSettingName;
            locationsArray[0] = locationName;
            return RunSimulationsAndSaveOutputs(farmSystemsArray, clockSettingsArray, locationsArray, simulationRootName, apsimDBTableName, netCDFFileName, outputsGroup, verboseEnabled);
        }
        ...
        public bool RunSimulationsAndSaveOutputs(IOMSimulationSetting simulationToRun, string simulationRootName,
                                                 string apsimDBTableName, string netCDFFileName, 
                                                 string outputsGroup = "LL", bool verboseEnabled = false)
        {
            ...
            { // run all simulations defined in simulationsSettings
                ...
                if (processedOK)
                {
                    ...
                    if (processedOK)
                    {
                        ...
                        {
                            iSim += 1;
                            string newSimulationName = simulationRootName + "(" + iSim + ")";
                            processedOK = CreateApsimSimulation(newSimulationName + ".apsimx", simToRun.FarmingSystemName, simToRun.ClockSettingName, simToRun.LocationName, simToRun.Name, verboseEnabled);
                            if (processedOK)
                            {
                                processedOK = RunApsimSimulation(newSimulationName + ".apsimx", true);
                                if (processedOK)
                                {
                                    System.Data.DataTable myData = null;
                                    if (outputsGroup == "LL")
                                    {
                                        myData = gatherSimulatedData(newSimulationName + ".db", apsimDBTableName, apsimRequiredLLVariables);
                                        if (myData != null)
                                        {
                                            processedOK = appendDataLLToNetCDF(myNetCDFConnection, myData);
                                        }
                                        else
                                            processedOK = false;
                                    }
                                    ...
                                    {
                                        myData = gatherSimulatedData(newSimulationName + ".db", apsimDBTableName, apsimRequiredIDVariables);
                                        if (myData != null)
                                        {
                                            processedOK = appendDataIDToNetCDF(myNetCDFConnection, myData);
                                        }
                                        else
                                            processedOK = false;
                                    }
                                    ...
                                    if (processedOK)
                                    { // clean up sim files
                                        processedOK = deleteFile(newSimulationName + ".apsimx");
                                        processedOK = deleteFile(newSimulationName + ".db");
                                    }
                                }
                            }
                        }
                        ...
                        // clean up met files
                        List<string> uniqueWeatherLocations = locationsSettings.Select(s => s.MetFileName).Distinct().ToList();
                        foreach (string metFile in uniqueWeatherLocations)
                        {
                            processedOK = deleteFile(metFile);
                        }
                    }
                }
            }
            else
            {
                ...
                if (processedOK)
                {
                    ...
                    if (processedOK)
                    {
                        processedOK = RunSimulationsAndSaveOutputs(simulationToRun.FarmingSystemName, simulationToRun.ClockSettingName,
                                                                   simulationToRun.LocationName, simulationRootName, apsimDBTableName,
                                                                   netCDFFileName, outputsGroup, verboseEnabled);
                        if (processedOK)
                        { // clean up files
                            processedOK = deleteFile(simulationRootName + "(0).apsimx");
                            processedOK = deleteFile(simulationRootName + "(0).db");
                            processedOK = deleteFile(simulationToRun.LocationName);
                        }
                ...
                foreach (var entry in mappedSettings.Children)
                {
                    ...
                    if (entry.Key.ToString().ToLower() == "outputsstructure")
                    {
                        ...
                        foreach (var innerEntry in (YamlMappingNode)entry.Value)
                        {
                            ...
                            {
                                dimensionsLLNames = innerEntry.Value.ToString().Split(separators, StringSplitOptions.RemoveEmptyEntries);
                            }
                            else if (innerEntry.Key.ToString() == "dimensionsUnits")
                            {
                                dimensionsLLUnits = innerEntry.Value.ToString().Split(separators, StringSplitOptions.RemoveEmptyEntries);
                            }
                            ...
                            {
                                apsimRequiredLLVariables = innerEntry.Value.ToString().Split(separators, StringSplitOptions.RemoveEmptyEntries);
                            }
                        }
                        ...
                        if (dimensionsLLNames.Length != dimensionsLLUnits.Length)
                        {
                        ...
                        if (apsimRequiredLLVariables.Length != (dimensionsLLNames.Length + outputVariablesNames.Length))
                        {
                        ...
                        if (verboseEnabled)
                        {
                            ...
                            for (int i = 0; i < dimensionsLLNames.Length; i++)
                            {
                                Console.WriteLine("    - {0} ({1})", dimensionsLLNames[i], dimensionsLLUnits[i]);
                            }
                            Console.WriteLine("  All these are based on apsim variables: " + string.Join(", ", apsimRequiredLLVariables));
                        }
                    }
                }
            }
                ...
                if (soilParameterisation.ContainsKey(parametersValue[0]))
                {
                    foreach (JObject soilInstance in modelsToSetUp)
                    {
                        foreach (KeyValuePair<string, JToken> item in soilInstance)
                        {
                            ...
                            if (soilParameterisation[parametersValue[0]][theKey] != null)
                            {
                                soilInstance[theKey] = soilParameterisation[parametersValue[0]][theKey];
                            }
                        }
                    }
                }
        ...
        public System.Data.DataTable gatherSimulatedData(string apsimDBFileName, string DBTableName, string[] variablesList = null)
        {
            ...
            {
                variablesList = apsimRequiredLLVariables;
            }
        ...
        private DataSet createNetCDFFileConnection(string outputDataFileName, string outputsGroup = "LL")
        {
            ...
            string[] dimensionsNames = null;
            string[] dimensionsUnits = null;
            if (outputsGroup == "LL")
            {
                dimensionsNames = dimensionsLLNames;
                dimensionsUnits = dimensionsLLUnits;
            }
            else if (outputsGroup == "ID")
            {
                dimensionsNames = dimensionsIDNames;
                dimensionsUnits = dimensionsIDUnits;
            }
            myFileConnection.Add<short[]>(dimensionsNames[i], dimensionsNames[i]);
            myFileConnection[dimensionsNames[i]].Metadata["FillValue"] = (short)missingValue;
            ...
            for (i = 1; i < dimensionsNames.Length; i++)
            {
                if (outputsGroup == "LL")
                {
                    ...
                    myFileConnection[dimensionsNames[i]].Metadata["FillValue"] = missingValue;
                }
                else if (outputsGroup == "ID")
                {
                    myFileConnection.Add<short[]>(dimensionsNames[i], dimensionsNames[i]);
                    myFileConnection[dimensionsNames[i]].Metadata["FillValue"] = "noID";
                    myFileConnection[dimensionsNames[i]].Metadata["comment"] = "unique identifier for location or ecotope (based on Annette's rendering of 2019-10-14)";
                }
                myFileConnection[dimensionsNames[i]].Metadata["standard_name"] = dimensionsNames[i];
                myFileConnection[dimensionsNames[i]].Metadata["units"] = dimensionsUnits[i];
            }
            ...
            for (i = 0; i < outputVariablesNames.Length; i++)
            {
                ...
                myFileConnection[outputVariablesNames[i]].Metadata["long_name"] = outputVariablesLongNames[i];
                myFileConnection[outputVariablesNames[i]].Metadata["cdsm_name"] = outputVariablesCDSMNames[i];
            ...
            // add some metadata
            myFileConnection.Metadata["title"] = "ApsimOutputs";
            myFileConnection.Metadata["description"] = "Outputs from Apsim model simulations";
            myFileConnection.Metadata["institution"] = "Plant and Food Research";
            myFileConnection.Metadata["conventions"] = "CF-1.7";
            myFileConnection.Metadata["source"] = "Apsim Simulations";
        ...
        private bool putDataToNetCDF(DataSet netCDFConnection, System.Data.DataTable simulatedData, string outputsGroup = "LL")
        {
            bool appendOK = true;

            // get the values for the time dimension and put them in the file
            ...
            netCDFConnection.PutData(dimensionsLLNames[0], timesSimulated);

            // get the values for the location dimension(s) and put them in the file
            int numDimensions = 0;
            if (outputsGroup == "LL")
            {
                numDimensions = dimensionsLLNames.Length;
                ...
                netCDFConnection.PutData(dimensionsLLNames[1], latitudesSimulated);
                netCDFConnection.PutData(dimensionsLLNames[2], longitudesSimulated);
            }
            else if(outputsGroup=="ID")
            {
                numDimensions = dimensionsIDNames.Length;
                short[] locationsSimulated = new short[] { (short)simulatedData.Rows[0]["LocationID"] };
                netCDFConnection.PutData(dimensionsIDNames[1], locationsSimulated);
            }

            // get the values for the variables and put them in the file
            for (int iVar = 0; iVar < outputVariablesNames.Length; iVar++)
            {
                string variableSimulatedName = apsimRequiredLLVariables[numDimensions + iVar];
                if (outputsGroup == "LL")
                {
                ...
                {
                    float[,] valuesSimulated = new float[numRows, 1];
                    for (int iRow = 0; iRow < numRows; iRow++)
                    {
                        valuesSimulated[iRow, 0] = float.Parse(simulatedData.Rows[iRow][variableSimulatedName].ToString());
                    }
        ...
        /// <summary>Appends the data (based on lat-long) from an Apsim simulation into a netCDF file.</summary>
        ...
        private bool appendDataLLToNetCDF(DataSet netCDFConnection, System.Data.DataTable simulatedData)
        {
            bool appendOK = true;
            int numRows = simulatedData.Rows.Count;
            ...
            if (timesRecorded.Length < 1)
            {
                ...
                netCDFConnection.Append(dimensionsLLNames[0], timesSimulated);
            }
            ...
            { // location with new latitude
                ...
                netCDFConnection.Append(dimensionsLLNames[1], latitudesSimulated[0]);
            }
            ...
            { // location with new longitude
                ...
                netCDFConnection.Append(dimensionsLLNames[2], longitudesSimulated[0]);
            }

            // get the values for the variables and put them in the file
            for (int iVar = 0; iVar < outputVariablesNames.Length; iVar++)
            {
                string variableSimulatedName = apsimRequiredLLVariables[dimensionsLLNames.Length + iVar];
                ...
                { // new simulated latitude, append new 'data row', add values for given location and set as missing for all others
                    ...
                    netCDFConnection.Append(outputVariablesNames[iVar], valuesSimulated, dimensionsLLNames[1]);
                }
                ...
                { // new simulated longitude, append new 'data column', add values to given location, and set as missing for all others
                    ...
                    netCDFConnection.Append(outputVariablesNames[iVar], valuesSimulated, dimensionsLLNames[2]);
                }
                else
        ...
        /// <summary>Appends the data (based on loc ID) from an Apsim simulation into a netCDF file.</summary>
        /// <remarks>Assumes that the time span for all simulations are the same (initialisation based on first simulation only).</remarks>
        /// <param name="netCDFConnection">Connection to the netCDF file.</param>
        /// <param name="simulatedData">DataTable with the simulated data.</param>
        /// <returns>True if data was appended, false otherwise.</returns>
        private bool appendDataIDToNetCDF(DataSet netCDFConnection, System.Data.DataTable simulatedData)
        {
            bool appendOK = true;
            int numRows = simulatedData.Rows.Count;
            int newDimensionFlag = 0; // flag which spatial dimension to append (0=none, 1=lat)
            ...
            if (timesRecorded.Length < 1)
            {
                ...
                netCDFConnection.Append(dimensionsIDNames[0], timesSimulated);
            }

            // check the values of the location dimension
            int iLoc = -1;
            short[] locationsRecorded = netCDFConnection.GetData<short[]>("location_id");
            short[] locationsSimulated = new short[] { Convert.ToInt16(simulatedData.Rows[0]["LocationID"]) };
            if (locationsRecorded.Contains(locationsSimulated[0]))
            { // location already recorded
                iLoc = Array.IndexOf(locationsRecorded, locationsSimulated[0]);
            }
            else
            { // new location ID
                newDimensionFlag += 1;
                iLoc = locationsRecorded.Length;
                netCDFConnection.Append(dimensionsIDNames[1], locationsSimulated[0]);
            }

            // get the values for the variables and put them in the file
            for (int iVar = 0; iVar < outputVariablesNames.Length; iVar++)
            {
                string variableSimulatedName = apsimRequiredIDVariables[dimensionsIDNames.Length + iVar];
                float[,] valuesSimulated;
                if (newDimensionFlag == 0)
                { // location given already exist, replace missing values with new values
                    valuesSimulated = netCDFConnection.GetData<float[,]>(outputVariablesNames[iVar]);
                    for (int iRow = 0; iRow < numRows; iRow++)
                    {
                        valuesSimulated[iRow, iLoc] = float.Parse(simulatedData.Rows[iRow][variableSimulatedName].ToString());
                    }

                    netCDFConnection.PutData(outputVariablesNames[iVar], valuesSimulated);
                }
                else if (newDimensionFlag == 1)
                { // new simulated location, append new 'data row', add values for given location and set as missing for all others
                    valuesSimulated = new float[numRows, 1];
                    for (int iRow = 0; iRow < numRows; iRow++)
                    {
                        valuesSimulated[iRow, 0] = float.Parse(simulatedData.Rows[iRow][variableSimulatedName].ToString());
                    }

                    netCDFConnection.Append(outputVariablesNames[iVar], valuesSimulated, dimensionsIDNames[1]);
                }
            }
        ...
        /// <summary>Delete a file from the output folder.</summary>
        /// <param name="fileName">Name of file to delete.</param>
        /// <returns>True if the file exists and was deleted, false otherwise.</returns>
        private bool deleteFile(string fileName)
        {
            bool deletedOk = true;
            string fileFullPath = Path.Combine(outputFilePath, fileName);
