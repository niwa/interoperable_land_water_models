...
namespace ApsimHandlerIOM
{
    ...
    public class MainHandler
    {
        ...
        private string[] dimensionsIDNames = { "time", "locationID" };
        ...
        private string[] dimensionsIDUnits = { "number of days since 1959-12-31", "Identifier for location or ecotope (based on rendering of 2019-09-25" };
...
/*                    if (processedOK)
                    {
                        int nCoresToUSe = 2;
                        Thread[] runningThreads = new Thread[2];
                        foreach (Thread givenThread in runningThreads)
                        {
                            //givenThread = new Thread(p => { runApsimSimulation(); });
                        }
                        for (int i = 0; i < 2; i++)
                        {
                            //runningThreads[i] = new Thread(new ParameterizedThreadStart(runApsimSimulation));
                        }

                        DataSet myNetCDFConnection = createNetCDFFileConnection(netCDFFileName, outputsGroup);
                        int nSims = simulationsSettings.Count;
                        for (int iS = 0; iS < nSims; iS++)
                        {
                            string newSimulationName = simulationRootName + "(" + iSim + ")";
        ...
        private bool runApsimSimulation(IOMSimulationSetting simulationToRun, string simulationName, string apsimDBTableName,
                                        DataSet myNetCDFConnection, string outputsGroup = "LL", bool verboseEnabled = false)
        {
            bool processedOK = false;
            processedOK = CreateApsimSimulation(simulationName + ".apsimx", simulationToRun.FarmingSystemName,
                                                simulationToRun.ClockSettingName, simulationToRun.LocationName,
                                                simulationToRun.Name, verboseEnabled);
            if (processedOK)
            {
                processedOK = RunApsimSimulation(simulationName + ".apsimx", true);
                if (processedOK)
                {
                    System.Data.DataTable myData = null;
                    if (outputsGroup == "LL")
                    {
                        myData = gatherSimulatedData(simulationName + ".db", apsimDBTableName, apsimRequiredLLVariables);
                        if (myData != null)
                        {
                            processedOK = appendDataLLToNetCDF(myNetCDFConnection, myData);
                        }
                    ...
                    {
                        myData = gatherSimulatedData(simulationName + ".db", apsimDBTableName, apsimRequiredIDVariables);
                        if (myData != null)
                        {
                            processedOK = appendDataIDToNetCDF(myNetCDFConnection, myData);
                        }
                    ...
                    if (processedOK)
                    { // clean up sim files
                        processedOK = deleteFile(simulationName + ".apsimx");
                        processedOK = deleteFile(simulationName + ".db");
                    }
                }
            }
        ...
        private DataSet createNetCDFFileConnection(string outputDataFileName, string outputsGroup = "LL")
        {
            ...
            for (i = 1; i < dimensionsNames.Length; i++)
            {
                ...
                {
                    myFileConnection.Add<string[]>(dimensionsNames[i], dimensionsNames[i]);
                    myFileConnection[dimensionsNames[i]].Metadata["MissingValue"] = "noID";
                }
                myFileConnection[dimensionsNames[i]].Metadata["Units"] = dimensionsUnits[i];
            }
        ...
        private bool putDataToNetCDF(DataSet netCDFConnection, System.Data.DataTable simulatedData, string outputsGroup = "LL")
        {
            ...
            else if(outputsGroup=="ID")
            {
                ...
                string[] locationsSimulated = new string[] { simulatedData.Rows[0]["LocationID"].ToString() };
        ...
        private bool appendDataIDToNetCDF(DataSet netCDFConnection, System.Data.DataTable simulatedData)
        {
            ...
            int iLoc = -1;
            string[] locationsRecorded = netCDFConnection.GetData<string[]>("locationID");
            string[] locationsSimulated = new string[] { simulatedData.Rows[0]["LocationID"].ToString() };
